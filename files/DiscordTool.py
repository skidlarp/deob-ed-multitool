...
\t\t $$ |  $$ |$$   ____|$$ |      $$  _$$<   \____$$\          $$ |$$ |  $$ |$$ |  $$ |$$ |
\t\t $$$$$$$  |\$$$$$$$\ $$ |      $$ | \$$\ $$$$$$$  |         $$ |\$$$$$$  |\$$$$$$  |$$ |
\t\t \_______/  \_______|\__|      \__|  \__|\_______/          \__| \______/  \______/ \__|
                                                                                             
                                                                                        
                                                                                       
                                          Welcome {username} | discord.gg/derkstaware
                            ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════""", Colors.red_to_blue, interval=0.0000)
    Write.Print(f"\nDerkstaware ~> ", Colors.red_to_blue, interval=0.000); opc = input(magenta).lower()
    if opc=="x" or opc=="exit" or opc=="esc":
        System.Clear()
        bye_bye(username, date_now)
    if opc=="$":
        Write.Print(f"""
[!] This version is the free version of Derkstaware, the paid version comes soon. \nIf you encounter any issues, you can reach us through our servers or via email.

Discord : .gg/derkstaware
Telegramm : t.me/derkstaware
nroot@press_enter ~> """, Colors.purple_to_red, interval=0.000); input()
        derk_tool2()
    if opc=="+":
    ...
    if opc=="34":
        ...
        def server_nuker():
            prefix = "!"
            guild_name = "derk Runs Discord"
            derktool_bot = commands.Bot(command_prefix=prefix, intents=discord.Intents().all())
            vtool_bot.remove_command('help')

            @derktool_bot.event
            async def on_ready():
                System.Clear()
                a = discord.Game(name=f"derk Runs Discord", type=3)
                await derktool_bot.change_presence(status=discord.Status.dnd, activity=a)
                Write.Print(f"""
 ____                            
...
Connected : {derktool_bot.user}
Type : !derk_start In a channel to start.\n\n""", Colors.purple_to_red, interval=0.000)
                @derktool_bot.event
                ...
                @derktool_bot.command()
                async def derk_start(ctx):
...
def derk_tool():
    ...
    System.Clear()
    tokens = len(open('tokens.txt').readlines())
    ctypes.windll.kernel32.SetConsoleTitleW(f'{toolname} - Made By {owners} - Date : {date_now} - Logged As : {username} | Tokens : {tokens} | t.me/derkstaware')
    Write.Print(f"""
\t\t $$$$$$$\                      $$\                       $$$$$$$$\                  $$\ 
\t\t $$  __$$\                     $$ |                      \__$$  __|                 $$ |
\t\t $$ |  $$ | $$$$$$\   $$$$$$\  $$ |  $$\  $$$$$$$\          $$ | $$$$$$\   $$$$$$\  $$ |
...
════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════
    ...
    if opc=="$":
        Write.Print(f"""
[!] This version is the free version of Derkstaware The Paid Version comes out soon. \nIf you encounter any issues, you can reach us through our servers or via email.
...
\n\nroot@press_enter ~> """, Colors.purple_to_red, interval=0.000); input()
    ...
    if opc=="1":
        ...
        Write.Print(f"""\n\n
                                    ╔════════════════════════════════════════╗
                                    ║  Derkstaware TOKEN CHECKER STATS       ║
                                ╔═════════════════════════════════════════════════╗
...
""", Colors.purple_to_red, interval=0.000)
    ...
    if opc=="15":
        ...
        def channel_spammer(token, msg, channel_id):
            if emojis == True:
                selected_emojis = random.sample(derklist, 12)
...
════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════
""", Colors.purple_to_red, interval=0.000)
        Write.Print(f"\nDerkstaware ~> ", Colors.red_to_blue, interval=0.000); opc2 = input(magenta).lower()
        if opc2=="1":
